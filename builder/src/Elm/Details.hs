...
crawlFile foreignDeps mvar pkg src docsStatus expectedName path =
  do  bytes <- File.readUtf8 path
               & Lamdera.alternativeImplementationPassthrough (Lamdera.PackageReplacements.get pkg expectedName)
...
crawlKernel foreignDeps mvar pkg src name =
  do  let path = src </> ModuleName.toFilePath name <.> "js"
      ...
      if exists
        then
          do  bytes <- File.readUtf8 path
                       & Lamdera.alternativeImplementationPassthrough (Lamdera.PackageReplacements.get pkg name)
