...
main = do
    ...
    phony "build" $ need [ "lamdera" ]
    phony "lamdera" $ need [ lamdera ]
    ...
    "generated/Build_lamdera.hs" %> \out -> do
        alwaysRerun
        writeFileChanged out $ unlines
            [ "module Build_lamdera where"
            , ""
            , "gitDescribe :: String"
    ...
    Shakefiles.Haskell.cabalProject "lamdera"
        [ "elm.cabal"
    ...
    Shakefiles.Haskell.executable lamdera "lamdera" gitDescribe
    ...
    forM_ elmVersions $ \elmVersion -> do
        ...
        ("_build/tests/test-files/*/Elm-" ++ elmVersion ++ "//*.elm_formatted") %> \out -> do
            ...
            need [ lamdera, source ]
            cmd_ lamdera source "--output" out ("--elm-version=" ++ elmVersion)

        ("_build/tests/test-files/*/Elm-" ++ elmVersion ++ "//*.elm_stderr") %> \out -> do
            let source = dropDirectory1 $ out -<.> "elm"
            need [ lamdera, source ]
            (Stderr stderr, Exit _) <- cmd (FileStdin source) BinaryPipes lamdera "--stdin" ("--elm-version=" ++ elmVersion)
    ...
    "_build/tests//*.elm_from_json" %> \out -> do
        ...
        need [ lamdera, source ]
        cmd_ (FileStdout out) lamdera "--from-json" source
    ...
    "_build/tests//*.json_formatted" %> \out -> do
        let source = dropDirectory1 $ out -<.> "elm"
        need [ lamdera, source ]
        (Stdout rawJson) <- cmd (FileStdin source) lamdera "--elm-version=0.19" "--stdin" "--json"
    ...
    "_build/tests//*.json_roundtrip_formatted" %> \out -> do
        ...
        need [ lamdera, source ]
        cmd_ (FileStdout out) lamdera "--elm-version=0.19" "--from-json" source
