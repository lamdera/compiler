...
module Lamdera.Wire3.Interfaces where
...
modifyModul pkg ifaces modul =
  unsafePerformIO $ do
    if (Lamdera.Wire3.Helpers.shouldHaveCodecsGenerated pkg)
      then do
          ...
          newModul =
            modul {
              Src._values =
                ...
                -- write the `expected_w3_[en|de]code_TYPENAME` style tests, which ensures our
...
unionStubs unions =
  unions
    & concatMap (\(A.At _ (Src.Union (A.At _ name) tvars _)) ->
      ...
      , _Debug_todo_with_params nParams $ Data.Name.fromChars $ "w3_decodeWithoutValidate_" ++ Data.Name.toChars name
      ]
    )
